"""Shared fixtures for Discount Code Request test suite."""
...
@pytest.fixture
def mock_route_to_discount(monkeypatch):
    """Force router to route to discount agent."""
    async def _route(state):
        state["intent"] = "Discount Code Request"
        state["routed_agent"] = "discount"
        return state
...
def payload_discount(
    ...
    **kwargs
    ...
    """Build a discount code request payload."""
    return {
        "conversation_id": conv_id or f"discount-{uuid.uuid4().hex[:8]}",
        "user_id": kwargs.get("user_id", "user-test"),
        "channel": kwargs.get("channel", "email"),
        "customer_email": email,
        "first_name": first_name,
        "last_name": last_name,
        "shopify_customer_id": kwargs.get("shopify_customer_id", "cust-discount"),
        "message": message,
    }
