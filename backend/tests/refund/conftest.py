"""Shared fixtures for Refund Request test suite."""
...
@pytest.fixture
def mock_route_to_refund(monkeypatch):
    """Force router to route to refund agent."""
    async def _route(state):
        state["intent"] = "Refund Request"
        state["routed_agent"] = "refund"
        return state
...
def payload_refund(
    ...
    **kwargs
    ...
    return {
        "conversation_id": conv_id or f"refund-{uuid.uuid4().hex[:8]}",
        "user_id": kwargs.get("user_id", "user-test"),
        "channel": kwargs.get("channel", "email"),
        "customer_email": email,
        "first_name": first_name,
        "last_name": last_name,
        "shopify_customer_id": kwargs.get("shopify_customer_id", "cust-refund"),
        "message": message,
    }
