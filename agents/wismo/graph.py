"""LangGraph-based micro-state machine for the WISMO workflow.
...
The graph operates on the shared `AgentState` TypedDict from `core.state`
and uses `internal_data` to store:
...
class WismoAgent(BaseAgent):
    ...
    `agents.wismo.graph.build_wismo_graph`.
    """
